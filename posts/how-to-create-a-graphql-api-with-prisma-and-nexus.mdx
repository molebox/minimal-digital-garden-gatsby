...
In `SQL` land relationships between tables are defined by foreign keys. These are ids that map to other tables. In the example above you can see that the `Industry` model has a field called `companies` which has a type of `Company[]`, indicating that it represents an array, or list, of companies. Checking the `Company` model it has its own `id` field and another id field called `industryId`. This field is a foreign key and indicates to the database a one-to-many relationship between the `Industry` and `Company` tables, where a industry can have many companies but a company will relate to one industry.

The `?` next to many of the field denotes that they are nullable. This is so that you can run mutations against them and not have to worry about updating all the data of a company, for example if you wanted to change the name of a company and only the name.
...
The API isn't much good if you can't make any requests against it. Begin by opening back up the `companies.ts` file. Copy the following into the file, replacing what you had before.
...
Now that's a big old code dump! Let's see if we can dissect it.

At the top of the file you define a `CompanyInputType` which is an `inputObjectType`
